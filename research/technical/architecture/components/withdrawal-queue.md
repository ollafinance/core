...
| **Key Functions (typed and role scoped)** | `function requestWithdrawal(address user,uint256 shares,uint256 assets,uint256 rate) returns(uint256)` — only `CORE_ROLE`; `function finalizeWithdrawals(uint256 available) returns(uint256)` — only `CORE_ROLE`; `function claim(uint256 id) returns(uint256)` — public; `function getRequest(uint256 id) view returns(WithdrawalRequest)` — view; `function nextUnfinalized() view returns(uint256)` — view |
