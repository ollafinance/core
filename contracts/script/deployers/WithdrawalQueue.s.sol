...
contract WithdrawalQueueDeployer is BaseDeployer {
    ...
    /// @param vault The OllaVault proxy address.
    ...
    function deploy(DeployConfig memory config, address vault, address admin, uint256 gasThreshold)
        external
        returns (address implementation, address proxy)
    {
        ...
        bytes memory initData = abi.encodeCall(WithdrawalQueue.initialize, (vault, admin, gasThreshold));
