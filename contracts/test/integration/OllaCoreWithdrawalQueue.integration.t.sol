...
contract OllaCoreFinalizedWithdrawalBugTest is Test {
    ...
    function test_FinalizedWithdrawal_CanBeClaimedAfterRebalance() external {
        ...
        // Step 6: Another rebalance - THIS IS WHERE THE BUG MANIFESTS
        // The finalized withdrawal funds should NOT be re-staked, but they are
        vault.rebalance();

        // Check if funds were incorrectly re-staked
        uint256 vaultBalanceAfterExtraRebalance = asset.balanceOf(address(vault));

        // This assertion will FAIL due to the bug - funds get re-staked
        assertGe(
        ...
        // Step 7: User tries to claim - this will revert with ERC20InsufficientBalance due to the bug
        vm.prank(alice);
        uint256 claimed = vault.claimRequestById(requestId);
