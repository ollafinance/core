...
/// @dev Uses MockRewardsCollector to properly test reward harvesting flow.
contract StakingManagerHarvestTest is StakingManagerBaseTest {
        ...
        returns (IStakingManager.KeyStore[] memory keys)
    {
        ...
        rollup.setRewards(address(rewardsCollector), totalRewards);
    ...
    function test_HarvestRewards_ClaimsRewardsToVault() external {
        ...
        assertEq(rollup.getSequencerRewards(address(rewardsCollector)), 0, "Vault rewards should be zero after harvest");
    }
    ...
    function test_HarvestRewards_IncreasesRewardsCollectorBalance() external {
        uint256 rewardAmount = 10 ether;
        _setupAttestersWithRewards(1, rewardAmount);

        uint256 vaultBalanceBefore = aztec.balanceOf(address(rewardsCollector));

        vm.prank(core);
        stakingManager.harvestRewards();

        uint256 vaultBalanceAfter = aztec.balanceOf(address(rewardsCollector));
        // Rewards are paid directly to the RewardsCollector by the rollup claim.
