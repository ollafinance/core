...
contract StakingManagerRollupUpgradeTest is StakingManagerBaseTest {
    ...
    function test_RefreshAttesterState_QueuedAttester_HandlesExitOnNewCanonicalAfterUpgrade() external {
        _setupStakedAttester();
        address attester = _attesterAddresses(1)[0];

        rollup.clearAttester(attester);
        rollup.setStake(attester, 0, address(0));
        rollupB.setExternalExit(attester, ACTIVATION_THRESHOLD, block.timestamp + 1 days);
        rollupRegistry.setCanonicalRollup(address(rollupB));

        stakingManager.refreshAttesterState(_attesterAddresses(1));

        IStakingManager.StakingState memory state = stakingManager.getStakingState();
        assertTrue(stakingManager.isUnstakePending(attester), "canonical exit transitions queued attester to exiting");
        assertEq(stakingManager.getActivatedAttesterCount(), 0, "attester should not become active");
        assertEq(stakingManager.getPendingUnstakeCount(), 1, "attester should be tracked as exiting");
        assertEq(state.stakedAmount, 0, "queued stake moved out of staked amount");
        assertEq(state.pendingUnstakeAmount, ACTIVATION_THRESHOLD, "canonical exit amount tracked as pending");
    }

    function test_RevertWhen_PurgeFailedQueueEntry_QueuedAttesterHasExitOnNewCanonicalAfterUpgrade() external {
        _setupStakedAttester();
        address attester = _attesterAddresses(1)[0];
    ...
    function test_RevertWhen_GetClaimableRewards_LegacyRewardReadReverts() external {
        ...
        vm.expectRevert(IMockAztecRollup.MockAztecRollup__ClaimFailed.selector);
        vm.prank(core);
