...
contract OllaCore is Initializable, IOllaCore {
    ...
    function requestRedeem(uint256 shares, address receiver, address owner) external override returns (uint256 assets) {
    ...
    function claimWithdraw(address owner) external override returns (uint256 assets) {
        ...
        internal
    {
        ...
        if (msg.sender != owner) {
            IERC20(address(_stAztec)).safeTransferFrom(owner, address(this), shares);
            _stAztec.burn(address(this), shares);
        } else {
