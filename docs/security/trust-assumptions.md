...
**OllaGovernance (timelock)** — holds `DEFAULT_ADMIN_ROLE` on all contracts; 2-day minimum delay on proposals controlling upgrades (`upgradeCore`, `upgradeSatellite`), the parameter setters timelocked through `OllaGovernance` (OllaCore passthroughs: `setTreasury`, `setProtocolFeeBP`, `setTreasuryFeeSplitBP`, `setRebalanceGasThreshold`, `setSafetyModule`, `setRebalanceCooldown`, `recoverStAztec`, `reconcileBufferedAssets`; SafetyModule passthroughs: `setDepositCap`, `setWithdrawalMinimum`, `setMinRateDropBps`, `setMaxQueueRatioBps`, `setMaxAccountingDelay`), governance transfer (`proposeGovernance`, `acceptGovernance`, `cancelGovernanceProposal`), and role grants; the setup-only `setCore` is timelocked but one-time. Emergency pause/unpause (`emergencyPauseAll`, `emergencyUnpauseAll`) executes instantly without timelock via the separate `governanceAdmin` role. This list mirrors `IOllaGovernance` / `OllaGovernance`; keep it in sync when that interface changes.
...
| T-9 | Providers/operators route the raw Aztec sequencer reward stream for Olla-managed validators to `RewardsAccumulator` (i.e. propose/prove with the coinbase set to `RewardsAccumulator`) | StakingManager, RewardsAccumulator, OllaCore | The Aztec rollup credits sequencer rewards to the block/proof coinbase, and Olla does not bind or verify that coinbase on-chain. A provider/operator that proposes with a different coinbase has all raw sequencer rewards accrue outside `RewardsAccumulator`, `totalAssets()`, and fee/share accounting, then claims them directly — denying that yield to stAztec holders. This captures the raw reward stream generated by user-funded stake before Olla accounts for it; it is broader than T-8's provider fee-recipient redirection. It does not take staked principal or vault buffer assets. |
...
- **T-9** is an operational trust assumption: the Aztec coinbase for Olla-managed validators is not enforced or verified on-chain today. It is mitigated by operational monitoring — any Olla-managed validator proposing/proving with a coinbase other than `RewardsAccumulator` should be treated as a slashable/provider-removal event — and by provider onboarding requirements that mandate `RewardsAccumulator` as the coinbase. Enforcing or persisting the expected coinbase on-chain at attester activation is tracked as a future hardening step.
